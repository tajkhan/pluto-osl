...
/*
 * in case of no input_dim, return 0
 */
int is_alpha( osl_relation_p scatt, int row) {
  int i = 0;
  for (i=0; i<scatt->nb_input_dims; i++)
    if(!osl_int_zero(scatt->precision, scatt->m[row][i+scatt->nb_output_dims+1]))
        return 1;
...
/*
 * If Pluto generates two consecutive rows of iterators (alphas)
 * in scattering, insert a blank beta between them.
 */
void pluto_insert_blank_betas (osl_scop_p scop) {
  osl_statement_p stm = NULL;
  int i = 0;

  if (scop==NULL)
    return;

  //calculate max scattering
  stm = scop->statement;
  //for each statement generate and add beta
  for ( ; stm; stm=stm->next) {
    //ASSUMPTION: coming from pluto, rows must be in order
    //if first row is alpha, insert beta before it
    if (is_alpha(stm->scattering, 0)) {
      osl_relation_insert_blank_row(stm->scattering, 0);
      osl_relation_insert_blank_column(stm->scattering, 0+1);
      osl_int_set_si(stm->scattering->precision, &stm->scattering->m[0][0+1], -1);
      stm->scattering->nb_output_dims += 1;
     }

    for (i=0; i< stm->scattering->nb_rows; i++) {
      //if two consecutive alphas, insert a blank beta in between
      if (is_alpha(stm->scattering, i)) {
        if (i+1==stm->scattering->nb_rows || is_alpha(stm->scattering, i+1)) {
          osl_relation_insert_blank_row(stm->scattering, i+1);
          osl_relation_insert_blank_column(stm->scattering, i+2);
          osl_int_set_si(stm->scattering->precision, &stm->scattering->m[i+1][i+2], -1);
          stm->scattering->nb_output_dims += 1;
        }
       }
    }
...
osl_strings_p generate_scatt_names(PlutoProg* prog) {
  ...
  for (i=0; i<prog->num_hyperplanes; i++) {
    sprintf(name, "t%i", num_t++);
    osl_strings_add (str, name);
    if (hProps[i].type==H_LOOP||hProps[i].type==H_TILE_SPACE_LOOP) {
      if (hProps[i+1].type==H_LOOP||hProps[i+1].type==H_TILE_SPACE_LOOP) {
        sprintf(name, "b%i", num_b++);
        osl_strings_add (str, name);
      }
    }
  }

  if (hProps[prog->num_hyperplanes-1].type==H_LOOP ||
      hProps[prog->num_hyperplanes-1].type==H_TILE_SPACE_LOOP) {
    sprintf(name, "b%i", num_b++);
    osl_strings_add (str, name);
  }
...
void pluto_populate_scop (osl_scop_p scop, PlutoProg *prog,
    ...
    osl_strings_p newnames = generate_scatt_names(prog);
